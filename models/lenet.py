'''
use lenet model and cifar data
'''
...
#ifrom continuous_evaluation import (train_acc_kpi, test_acc_kpi,
...
def run_benchmark(model, args):
    ...
    print ("tensor %s" % batch_size_tensor)
    ...
    train_reader = paddle.batch(
        paddle.dataset.cifar.train10(), batch_size=args.batch_size)
    ...
    for pass_id in range(args.pass_num):
        ...
        for batch_id, data in enumerate(train_reader()):
            img_data = np.array(
                map(lambda x: x[0].reshape([3, 32, 32]), data)).astype(DTYPE)
            y_data = np.array(map(lambda x: x[1], data)).astype("int64")
            y_data = y_data.reshape([len(y_data), 1])
        ...
        print("pass=%d, cost_avg=%f" %
              (pass_id, np.mean(all_pass_cost)))
